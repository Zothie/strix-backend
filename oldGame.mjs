...
const validateAndTransformValue = (value, elementID, templates) => {
  ...
  const template = templates.statistics.find(
    (template) => template.templateCodeName === elementID
  );
  ...
  switch (templateType) {
    ...
    case "integer":
      ...
      if (isNaN(intValue)) {
        throw new Error(
          `Invalid value passed to element. Element '${template.templateName}' is '${template.templateType}', passed value '${value}' is not a number`
        );
      }
      return intValue;
    case "float":
      ...
      if (isNaN(floatValue)) {
        throw new Error(
          `Invalid value passed to element. Element '${template.templateName}' is '${template.templateType}', passed value '${value}' is not a float`
        );
      }
      return floatValue;
    case "bool":
      ...
      if (lowerCaseValue === "true") {
        ...
        throw new Error(
          `Invalid value passed to element. Element '${template.templateName}' is '${template.templateType}', passed value '${value}' is not "true" or "false"`
        );
      }
    default:
...
app.get("/api/sdk/getAllEntitiesID", async (req, res) => {
  try {
    ...
    const nodes = await NodeModel.find({
      "branches.planningTypes.nodes.entityProperties.entityID": {
        $exists: true,
      },
      gameID,
    }).exec();
  ...
  try {
    ...
    const selectedNode = resultNode.branches.reduce((acc, branch) => {
      const foundNode = branch.planningTypes.reduce((nodeAcc, planningType) => {
        const entityNode = planningType.nodes.find(
          (node) => node.entityProperties.entityID === entityID
        );
    ...
    if (!selectedNode) {
      return res
        .status(404)
        .json({ message: "Entity not found in the specified branch" });
    }
  ...
  try {
    ...
    if (!playerWarehouse) {
      playerWarehouse = await PlayerWarehouse.create({
        gameID,
        branches: [
          { branch: branchName, templates: { statistics: [] }, players: [] },
        ],
      });
    } else {
    ...
    let player = playerWarehouse.branches
    ...
    try {
      const transformedValue = validateAndTransformValue(
        value,
        elementID,
        playerWarehouse.branches.find((b) => b.branch === branchName).templates
      );

      // Найти элемент в elements.statistics с соответствующим elementID
      const elementIndex = player.elements.statistics.findIndex(
      ...
      if (elementIndex === -1) {
        ...
        player.elements.statistics[elementIndex].elementValue =
          transformedValue;
      }
      ...
      res
        ...
        .json({
          success: true,
          message: "Warehouse data element updated successfully",
        });
  ...
  try {
    ...
    try {
      ...
      if (elementIndex === -1) {
        ...
        if (
          template.templateType.toLowerCase() === "string" ||
          template.templateType.toLowerCase() === "bool"
        ) {
          throw new Error(
            `Cannot perform "add" operation on '${template.templateType}' element type`
          );
        }
...
app.post("/api/sdk/getWarehouseDataElement", async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Warehouse data element sent successfully",
  ...
  try {
    const events = await AnalyticsEvents.find(
      {
        ...
        "branches.events.eventCodeName": eventID,
      },
      { "branches.$": 1 }
    );

    const matchingEvent = events.find(
      (event) => event.branches[0].events[0].eventCodeName === eventID
    );
    ...
    if (matchingEvent) {
      const eventDetails = {
        ...
        // Grabbing values
        valueID1:
          matchingEvent.branches[0].events[0].values[0] !== undefined
            ? matchingEvent.branches[0].events[0].values[0]._id.toString()
            : "",
        valueFormat1:
          matchingEvent.branches[0].events[0].values[0] !== undefined
            ? matchingEvent.branches[0].events[0].values[0].valueFormat
            : "",

        valueID2:
          matchingEvent.branches[0].events[0].values[1] !== undefined
            ? matchingEvent.branches[0].events[0].values[1]._id.toString()
            : "",
        valueFormat2:
          matchingEvent.branches[0].events[0].values[1] !== undefined
            ? matchingEvent.branches[0].events[0].values[1].valueFormat
            : "",

        valueID3:
          matchingEvent.branches[0].events[0].values[2] !== undefined
            ? matchingEvent.branches[0].events[0].values[2]._id.toString()
            : "",
        valueFormat3:
          matchingEvent.branches[0].events[0].values[2] !== undefined
            ? matchingEvent.branches[0].events[0].values[2].valueFormat
            : "",
      };
    ...
    console.error(
      `Error fetching eventID ${eventID} for branch "${branchName}" for game ${gameID}:`,
      error
    );
  }
}
...
async function createNewPlayer(gameID, branchName, clientID) {
  try {
    ...
    if (!existingPlayer) {
      ...
      console.log(
        `Successfully created new player for gameID: ${gameID}, branchName: ${branchName}, clientID: ${clientID}`
      );
      return true;
    } else {
      console.log(
        `Player with clientID ${clientID} already exists in gameID: ${gameID}, branchName: ${branchName}`
      );
      return false;
    }
  } catch (error) {
...
async function recalculateSegments(
  ...
  try {
    ...
    const matchingSegments = segments.reduce((acc, curr) => {
      const matchingBranch = curr.branches.find(
        (branch) => branch.branch === branchName
      );
      if (matchingBranch) {
        const matchingSegmentConditions = matchingBranch.segments.filter(
          (segment) =>
            segment.segmentConditions.some(
              (condition) =>
                condition.conditionElementID === changedTemplate.templateID
            )
        );
    ...
    const playerObject = await getPlayerByClientID(
      gameID,
      branchName,
      clientID
    );
    ...
    const recalc = matchingSegments.forEach((segment) => {
      ...
      if (check) {
        segmentsLib.addSegmentToPlayer(
          gameID,
          branchName,
          clientID,
          segment.segmentID
        ...
        segmentsLib.removeSegmentFromPlayer(
          gameID,
          branchName,
          clientID,
          segment.segmentID
...
async function getPlayerByClientID(gameID, branchName, clientID) {
  try {
    ...
    const player = branch.players.find(
...
function transformValue(valueFormat, value) {
  switch (valueFormat) {
    ...
    case "integer":
      ...
      if (isNaN(intValue)) {
        throw new Error(
          `Invalid value. Expected an integer, but received '${value}'`
        );
      }
      return intValue;
    case "float":
    ...
    case "percentile":
      ...
      if (
        ...
        throw new Error(
          `Invalid value. Expected a percentile (integer between 0 and 100), but received '${value}'`
        );
      }
...
async function checkForTemplatesByDesignEvent(
  gameID,
  branchName,
  event,
  clientID,
  ...
  async function findAnalyticsTemplatesByEventID(gameID, branchName, eventID) {
    try {
      const foundTemplates = await PlayerWarehouse.aggregate([
        ...
        {
          $match: {
            "branches.templates.analytics.templateAnalyticEventID": eventID,
          },
        },
        {
          $group: {
            _id: null,
            templates: { $push: "$branches.templates.analytics" },
          },
        },
      ]);

      return foundTemplates && foundTemplates.length > 0
        ? foundTemplates[0].templates
        : [];
    } catch (error) {
      console.error("Error finding analytics templates:", error);
    }
  }
  const foundTemplates = await findAnalyticsTemplatesByEventID(
    gameID,
    branchName,
  ...
  if (foundTemplates && foundTemplates.length > 0) {
    for (const foundTemplate of foundTemplates) {
      ...
      if (
        foundTemplate.templateEventTargetValueId === event.eventDetails.valueID1
      ) {
        correspondingEventValue = value1;
        eventValueFormat = event.eventDetails.valueFormat1;
        valueColumnName = "value1";
      } else if (
        foundTemplate.templateEventTargetValueId === event.eventDetails.valueID2
      ) {
        correspondingEventValue = value2;
        eventValueFormat = event.eventDetails.valueFormat2;
        valueColumnName = "value2";
      } else if (
        foundTemplate.templateEventTargetValueId === event.eventDetails.valueID3
      ) {
      ...
      const conditionsPassed = checkTemplateConditions(
        foundTemplate,
        event,
        value1,
        value2,
        ...
        switch (foundTemplate.templateMethod) {
          ...
          case "meanForTime":
            ...
            const meanForTimeValue = await druidLib.getMeanValueForTime(
              ...
              valueColumnName,
              isFloat,
              foundTemplate.templateMethodTime
            );
            playerWarehouseLib.setElementValue(
              gameID,
              branchName,
          ...
          case "numberOfEvents":
            playerWarehouseLib.incrementElementValue(
              gameID,
              branchName,
              clientID,
              foundTemplate.templateID
          ...
          case "summ":
            playerWarehouseLib.addSummToElementValue(
              gameID,
              branchName,
              clientID,
              foundTemplate.templateID,
...
app.post("/api/sdk/analytics/newSession", async (req, res) => {
  ...
  try {
    ...
    if (!gameID || gameID === false) {
      res
        .status(500)
        .json({ success: false, message: "Invalid secretKey value" });
    }
    ...
    const playerCreated = await createNewPlayer(
      gameID,
      "development",
      clientID
    );
    ...
    res
      .status(200)
      .json({ success: true, message: "New session event acquired" });
  } catch (error) {
...
app.post("/api/sdk/analytics/endSession", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "End session event acquired" });
  } catch (error) {
  ...
  try {
    ...
    if (!foundEvent || !foundEvent.found) {
      res
        .status(404)
        .send(
          `Event with ID "${eventID}" was not found. Doublecheck your naming on webpage and inside game engine`
        );
      return;
    }
    ...
    try {
      sanitizedValue1 = transformValue(
        foundEvent.eventDetails.valueFormat1,
        value1
      );
      sanitizedValue2 = transformValue(
        foundEvent.eventDetails.valueFormat2,
        value2
      );
      sanitizedValue3 = transformValue(
        foundEvent.eventDetails.valueFormat3,
        value3
      );
    } catch (error) {
    ...
    const rerouteData = {
      ...
      engineVersion,
      buildType,
      value1: sanitizedValue1,
      value2: sanitizedValue2,
      value3: sanitizedValue3,
    };
...
app.post("/api/sdk/getRemoteConfigParam", async (req, res) => {
  try {
    ...
    const [player, remoteConfig] = await Promise.all([
      PlayerWarehouse.findOne({
        gameID,
        "branches.branch": branchName,
        "branches.players.clientID": clientID,
      }),
      RemoteConfig.findOne({
        gameID,
        "branches.branch": branchName,
        "branches.params.paramCodeName": paramID,
      }),
    ]);
    ...
    const segments = player.branches
      .find((b) => b.branch === branchName)
      .players.find((p) => p.clientID === clientID).segments;

    const param = remoteConfig.branches
      .find((b) => b.branch === branchName)
      .params.find((p) => p.paramCodeName === paramCodeName);
    ...
    function getSegmentValue(segments, values) {
      ...
      const everyoneObject = values.find(
        (value) => value.segmentID === "everyone"
      );
    ...
    res
      .status(200)
      .json({ success: true, value: resultValue, valueType: param.valueType });
  } catch (error) {
